...
// @desc    First-time Super Admin setup (only works if no super_admin exists)
// @route   POST /api/auth/setup-admin
// @access  Public (one-time bootstrap)
const setupAdmin = async (req, res) => {
    try {
        ...
        if (existingAdmin) {
            return res.status(403).json({ success: false, message: 'Super admin already exists. Please log in.' });
        }
