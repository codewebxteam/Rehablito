...
export const AttendanceView = ({ initialData }: { initialData?: any }) => {
  const [staff, setStaff] = useState<StaffMember[]>([]);
  const [attendance, setAttendance] = useState<ApiAttendance[]>(initialData?.attendance || []);
  ...
  const markStatus = async (row: AttendanceRow, uiStatus: 'Present' | 'Absent') => {
    if (!row.branchId) {
      toast.error('Therepist member has no branch assigned');
      return;
    }
    try {
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-low/30">
                  <th className="px-4 sm:px-6 py-4 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider opacity-70">Therepist Member</th>
