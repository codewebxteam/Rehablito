...
import { Lock, Mail, Eye, EyeOff, ArrowRight, HeartPulse, Stethoscope } from "lucide-react";
import SplitLayout from "../../components/SplitLayout";
import { useAuth } from "../../context/AuthContext";

export default function ParentLoginPage() {
  const { parentLogin, loading } = useAuth();
  ...
  const leftContent = (
    <div className="absolute bottom-12 left-0 max-w-xl p-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}
        ...
        <span className="w-2 h-2 rounded-full bg-brand-rose shadow-[0_0_12px_rgba(252,231,243,1)]"></span>
        <span className="text-white text-[10px] font-bold uppercase tracking-widest">Parents Portal</span>
      </motion.div>
      <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
        ...
        Track your child's <br /><span className="text-brand-rose">progress daily.</span>
      </motion.h1>
      <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}
        className="text-white/80 text-lg leading-relaxed max-w-md font-medium">
        Stay updated on therapy sessions, attendance, and communicate directly with your branch manager.
      </motion.p>
    </div>
  );
    ...
    <div className="w-full max-w-sm mx-auto">
      <div className="mb-8 text-center sm:text-left">
        ...
        <div className="flex items-center justify-center sm:justify-start gap-2 mb-2">
          <HeartPulse className="w-5 h-5 text-brand-sage" />
          <span className="text-xs font-bold uppercase tracking-widest text-brand-sage">Parents Portal</span>
        </div>
        <h2 className="text-2xl sm:text-3xl font-bold text-on-background tracking-tight">Welcome Back</h2>
        <p className="text-on-surface-variant mt-2 font-medium text-sm sm:text-base">Sign in to monitor progress.</p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative group">
            ...
            <input
              ...
              type="email" autoCapitalize="none" autoCorrect="off" autoComplete="email" placeholder="parent@example.com" value={email}
        ...
        <div className="space-y-2">
          ...
          <div className="relative group">
            ...
            <input
              ...
              type={showPassword ? "text" : "password"} autoCapitalize="none" autoCorrect="off" autoComplete="current-password" placeholder="••••••••" value={password}
  ...
  return (
    <SplitLayout
      ...
      leftBgImage="https://images.unsplash.com/photo-1576765608535-5f04d1e3f289?q=80&w=2069&auto=format&fit=crop"
    />
  );
}
